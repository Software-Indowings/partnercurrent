    ...
    <div className='d-flex justify-content-center align-items-center vh-100' style={{ backgroundImage: `url(${backgroundImage})`, backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div className='w-75 bg-white rounded p-5'>
        ...
        <div className='d-flex justify-content-end mb-4'>
          <Link to="/create_products" className='btn btn-success'> Create +</Link>
        </div>
          ...
          <table className='table table-bordered table-hover' style={{ borderCollapse: 'collapse', width: '100%' }}>
            ...
            <tbody>
              {data.map((product, index) => {
                return (
                  <tr key={index} style={{ backgroundColor: index % 2 === 0 ? '#fff' : '#f9f9f9' }} className="table-row-hover">
                    <td style={{ padding: '10px' }}>{product.product_id}</td>
                    <td style={{ padding: '10px' }}>{product.name}</td>
                    <td style={{ padding: '10px' }}><a href={product.brochure} target="_blank" rel="noopener noreferrer">View Brochure</a></td>
                    ...
                    <td style={{ padding: '10px' }}>
                      <Link to={`/read_products/${product.product_id}`} className='btn btn-sm btn-info me-2'>Manage</Link>
        ...
        <Link to="/adminpage" className='btn btn-primary'>Back</Link>
      </div>
    </div>
  );
}
